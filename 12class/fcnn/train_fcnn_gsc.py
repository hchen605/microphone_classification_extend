...
gsc_class = ['down', 'go', 'left', 'no', 'off', 'on', 'right', 'stop', 'up', 'yes', '_unknown_', '_silence_']
# -
...
if gsc:
    train_csv = '../data/train_gsc.csv'
    dev_csv = '../data/dev_gsc.csv'
    test_csv = '../data/test_gsc.csv'
else:
...
if gsc==1:
    ...
    train = load_data_gsc(train_csv)
    dev = load_data_gsc(dev_csv)
    test = load_data_gsc(test_csv)
else:
...
# Model
model = model_fcnn_pre(num_classes, input_shape=[num_freq_bin, None, num_audio_channels], num_filters=[24, 48, 96], wd=0)
...
file1 = open("record_gsc_pre_{}_{}.txt".format(num_classes, args.limit), "a")  
